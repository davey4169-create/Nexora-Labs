...
function normalizeDb(db) {
  return {
    ...
    affiliateAccounts: Array.isArray(db?.affiliateAccounts) ? db.affiliateAccounts : [],
...
function userAffiliateAccount(db, userId) {
  return (db.affiliateAccounts || []).find((item) => item.userId === userId) || null;
}

function userWithdrawals(db, userId) {
  return (db.withdrawals || []).filter((item) => item.userId === userId);
}
...
function mergeAdminPayload(current, payload) {
  const next = normalizeDb({
    ...
    affiliateAccounts: Array.isArray(payload.affiliateAccounts) ? payload.affiliateAccounts : current.affiliateAccounts,
    withdrawals: Array.isArray(payload.withdrawals) ? payload.withdrawals : current.withdrawals,
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/api/health" && req.method === "GET") {
      ...
      let databaseError = "";
      try {
        db = await withTimeout(readDb(), DB_HEALTH_TIMEOUT_MS, "Database health check");
      } catch (error) {
        databaseOk = false;
    ...
    if (url.pathname === "/api/auth/register" && req.method === "POST") {
      ...
      if (!payload.fullName || !username || !email || !phone) {
      ...
      if (db.users.some((user) => cleanPhone(user.phone) === cleanPhone(phone))) {
        sendJson(res, 409, { error: "That phone number is already registered." });
        return;
      }
      ...
      sendJson(res, 200, { token, user: safeUser(user), notifications: [], affiliateAccount: null, withdrawals: [] });
      return;
    }
    ...
    if (url.pathname === "/api/auth/login" && req.method === "POST") {
      ...
      sendJson(res, 200, {
        ...
        affiliateAccount: userAffiliateAccount(db, user.id),
        withdrawals: userWithdrawals(db, user.id)
      });
      return;
    }
    ...
    if (url.pathname === "/api/profile" && req.method === "POST") {
      ...
      if (db.users.some((item) => item.id !== user.id && cleanPhone(item.phone) === cleanPhone(phone))) {
        sendJson(res, 409, { error: "That phone number is already used." });
        return;
      }
      Object.assign(user, {
        fullName: String(payload.fullName || "").trim(),
    ...
    if (url.pathname === "/api/password" && req.method === "POST") {
      ...
      user.passwordDigest = createPasswordDigest(payload.newPassword);
      delete user.passwordHash;
    ...
    if (url.pathname === "/api/affiliate/account" && req.method === "POST") {
      const db = await readDb();
      const user = sessionUser(req, db);
      if (!user) {
        sendJson(res, 401, { error: "Login required." });
      ...
      const payload = await readBody(req);
      const account = {
        id: userAffiliateAccount(db, user.id)?.id || createToken("acct"),
        userId: user.id,
        username: user.username,
      ...
      db.affiliateAccounts = (db.affiliateAccounts || []).filter((item) => item.userId !== user.id);
      db.affiliateAccounts.unshift(account);
      await writeDb(db);
      sendJson(res, 200, { account });
      return;
    }

    if (url.pathname === "/api/affiliate/withdraw" && req.method === "POST") {
      const db = await readDb();
      const user = sessionUser(req, db);
      if (!user) {
        sendJson(res, 401, { error: "Login required." });
      ...
      if (!amount || amount < 1000) {
        sendJson(res, 400, { error: "Enter a withdrawal amount of at least ₦1,000." });
        return;
      }
      const account = userAffiliateAccount(db, user.id);
      if (!account?.accountNumber) {
        sendJson(res, 400, { error: "Add payout account details before requesting withdrawal." });
        return;
      }
      const withdrawal = {
        id: createToken("wd"),
      ...
      await writeDb(db);
      sendJson(res, 200, { withdrawal, withdrawals: userWithdrawals(db, user.id) });
      return;
    }

    if (url.pathname === "/api/affiliate/verify-bank" && req.method === "POST") {
      if (!paystackSecretKey) {
        sendJson(res, 400, { error: "Paystack secret key is not configured." });
        return;
      }
      const db = await readDb();
      const user = sessionUser(req, db);
      ...
      const payload = await readBody(req);
      const accountNumber = encodeURIComponent(String(payload.accountNumber || "").trim());
      const bankCode = encodeURIComponent(String(payload.bankCode || "").trim());
      const response = await fetch(`https://api.paystack.co/bank/resolve?account_number=${accountNumber}&bank_code=${bankCode}`, {
        headers: { Authorization: `Bearer ${paystackSecretKey}` }
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok || !data.status) {
        sendJson(res, 400, { error: data.message || "Account verification failed." });
        return;
      }
      sendJson(res, 200, { accountName: data.data?.account_name || "" });
      return;
    }
    ...
    if (url.pathname === "/api/ai" && req.method === "POST") {
      ...
      if (!message) {
        sendJson(res, 400, { error: "Ask Nexora AI a question." });
        return;
      }
      if (!openAiApiKey) {
        sendJson(res, 200, {
          reply:
            "Nexora AI is ready, but the OPENAI_API_KEY is not configured yet. Ask the admin to add it in Render environment variables."
        });
        return;
      }
      const response = await fetch("https://api.openai.com/v1/responses", {
        ...
        body: JSON.stringify({
          model: openAiModel,
          instructions:
            "You are Nexora AI, a concise assistant for Nexora Labs. Help users understand services, courses, real estate inquiries, earning opportunities, and how to contact Nexora. Do not request passwords or private payment secrets.",
          input: message
        })
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) {
        sendJson(res, 400, { error: data.error?.message || "Nexora AI could not respond." });
        return;
      }
      sendJson(res, 200, { reply: data.output_text || "I could not generate a response right now." });
      return;
    }
    ...
    if (url.pathname === "/api/admin/user/delete" && req.method === "POST") {
      if (!isAdminSession(req)) {
        sendJson(res, 401, { error: "Admin login required." });
        return;
      }
      ...
      db.affiliateAccounts = db.affiliateAccounts.filter((item) => item.userId !== userId);
      db.withdrawals = db.withdrawals.filter((item) => item.userId !== userId);
      sendJson(res, 200, adminState(await writeDb(db)));
      return;
    }

    if (url.pathname === "/api/admin/user/message" && req.method === "POST") {
      if (!isAdminSession(req)) {
        sendJson(res, 401, { error: "Admin login required." });
        return;
      }
      ...
      if (!user || !message) {
        sendJson(res, 400, { error: "Select a user and write a message." });
        return;
      }
      db.notifications.unshift({
        id: createToken("note"),
        userId,
        title: "Nexora message",
        message,
        read: false,
        alerted: false,
        createdAt: new Date().toISOString()
    ...
    if (url.pathname === "/api/admin/user/reset-password" && req.method === "POST") {
      if (!isAdminSession(req)) {
        sendJson(res, 401, { error: "Admin login required." });
        return;
      }
      ...
      if (newPassword.length < 8) {
        sendJson(res, 400, { error: "Temporary password must be at least 8 characters long." });
        return;
      }
      user.passwordDigest = createPasswordDigest(newPassword);
      delete user.passwordHash;
      user.updatedAt = new Date().toISOString();
      db.notifications.unshift({
        id: createToken("note"),
        ...
        message: "Your Nexora account password was updated by admin. Login with the new password shared with you privately.",
        read: false,
        alerted: false,
        createdAt: new Date().toISOString()
      });
    ...
    if (url.pathname === "/api/admin/withdrawal/status" && req.method === "POST") {
      if (!isAdminSession(req)) {
        sendJson(res, 401, { error: "Admin login required." });
        return;
      }
      const db = await readDb();
      const payload = await readBody(req);
      const withdrawal = db.withdrawals.find((item) => item.id === payload.id);
      if (!withdrawal) {
        sendJson(res, 404, { error: "Withdrawal request not found." });
        return;
      }
      withdrawal.status = ["pending", "successful", "declined"].includes(payload.status) ? payload.status : "pending";
