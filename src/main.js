...
function bankSelect(account = {}) {
  return `
    <label>Bank
      <select name="bankPick" required>
        <option value="">Choose bank</option>
        ${NIGERIAN_BANKS.map((bank) => `<option value="${attr(`${bank.code}|${bank.name}`)}"${account.bankCode === bank.code ? " selected" : ""}>${esc(bank.name)}</option>`).join("")}
      </select>
    </label>
  `;
}
    ...
    .map(
      (item) => `
        <div class="earn-row">
          ...
          <strong class="${item.status === "successful" ? "status-success" : ""}">${esc(item.status)}</strong>
    ...
    <section class="content-band reveal" id="earnDashboard">
      ...
      <div class="earn-dashboard tilt-card">
        ...
        <div class="earn-panels">
          ...
          <form class="earn-panel" data-withdrawal-request>
            ...
            return `
              <article class="admin-card request-card user-admin-card" data-admin-user-card data-user-id="${attr(user.id)}" data-search="${attr(searchText)}">
                ...
                <div class="admin-detail-grid">
                  ...
                  <p><i data-lucide="dollar-sign"></i> Pending: ${money(pending)} / Successful: ${money(paid)}</p>
                </div>
              ...
              <article class="admin-card request-card" data-withdrawal-id="${attr(item.id)}">
                ...
                <label>Status
                  <select data-withdrawal-status>
                    ${["pending", "successful"].map((status) => `<option${item.status === status ? " selected" : ""}>${status}</option>`).join("")}
                  </select>
                </label>
...
function initEarnDashboard() {
  ...
  if (payoutForm) {
    ...
    payoutForm.querySelector('select[name="bankPick"]')?.addEventListener("change", scheduleAutoVerify);
...
async function apiRequest(path, options = {}) {
  const bases = [apiBaseUrl, "", "http://localhost:5174", "http://127.0.0.1:5174"].filter((value, index, array) => array.indexOf(value) === index);
  ...
  for (const base of bases) {
    try {
      ...
      if (!contentType.includes("application/json")) throw new Error("Invalid API response.");
