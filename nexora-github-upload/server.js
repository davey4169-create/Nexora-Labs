...
async function readDb() {
  ...
  if (client) {
    const result = await client.query("SELECT data FROM nexora_state WHERE id = $1", ["main"]);
...
function listen(port) {
  ...
  server.listen(port, host, () => {
    console.log(`Nexora server running at http://${host === "0.0.0.0" ? "127.0.0.1" : host}:${port}/`);
